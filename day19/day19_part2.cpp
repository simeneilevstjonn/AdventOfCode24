...
std::vector<long long> times_for_length = std::vector<long long>();

long long count(const char* pattern) 
{   
    long long c = 0;
    if (!*pattern) 
    {
    ...
    if (times_for_length[strlen(pattern)] != -1)
    {
        // std::cout << "size is " << strlen(pattern) << " returning due to impossible offset\n";
        return times_for_length[strlen(pattern)];
    }

    for (const char* towel : towels) 
    {
        if (prefix_match(pattern, towel)) 
        {
            c += count(pattern + strlen(towel));
        }
    }

    times_for_length[strlen(pattern)] = c;
    return c;
}
...
int main() 
{
    ...
    for (std::string s; std::cin >> s; )
    {
        ...
        times_for_length.resize(s.size() + 1);
        std::fill(times_for_length.begin(), times_for_length.end(), -1);

        long long res = count(s.c_str());
