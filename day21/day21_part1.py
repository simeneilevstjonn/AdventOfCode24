    ...
    def init_edges(self):
        ...
        if self.layer == 0:
            if self.y > 0 and not (self.y == 1 and self.x == 0):
                self.edges.append([1, keypad_vertices[self.y - 1][self.x][self.layer]])
        elif self.layer == 1:
            if self.x < 2:
                self.edges.append([1, keypad_vertices[self.y][self.x + 1][self.layer]])
        elif self.layer == 2:
            if self.y < 1:
                self.edges.append([1, keypad_vertices[self.y + 1][self.x][self.layer]])
        elif self.layer == 3:
        ...
        for other_layer in range(5):
            ...
            self.edges.append([manhattan(*layer_to_coords(self.layer), *layer_to_coords(other_layer)), keypad_vertices[self.y][self.x][other_layer]])

keypad_vertices = [[[KeypadVertex(i, j, k) for k in range(5)] for j in range(3)] for i in range(2)]

keypad_vertices_as_list = []

for plane in keypad_vertices:
    for row in plane:
        for col in row:
            col.init_edges()
